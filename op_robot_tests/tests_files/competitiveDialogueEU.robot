...
*** Variables ***
${mode}         competitive_dialogue_eu
@{used_roles}   tender_owner  provider  provider1  viewer


*** Test Cases ***
Можливість оголосити понадпороговий однопредметний конкурентний діалог
  [Tags]   ${USERS.users['${tender_owner}'].broker}: Можливість оголосити конкурентний діалог
  ...      tender_owner
...
Пошук позапорогового однопредметного конкурентний діалог по ідентифікатору
  [Tags]   ${USERS.users['${viewer}'].broker}: Пошук конкурентного діалогу по ідентифікатору
...
Відображення типу закупівлі оголошеного конкурентного діалогу
  [Tags]   ${USERS.users['${viewer}'].broker}: Відображення основних даних оголошеного конкурентного діалогу
  ...      viewer
  ...      ${USERS.users['${viewer}'].broker}
  Звірити поле тендера  ${viewer}  ${USERS.users['${tender_owner}'].initial_data}  procurementMethodType

Відображення початку періоду прийому пропозицій оголошеного конкурентного діалогу
  [Tags]   ${USERS.users['${viewer}'].broker}: Відображення основних даних оголошеного конкурентного діалогу
  ...      viewer
  ...      ${USERS.users['${viewer}'].broker}
  ...      minimal
...
Відображення закінчення періоду прийому пропозицій оголошеного конкурентного діалогу
  [Tags]   ${USERS.users['${viewer}'].broker}: Відображення основних даних оголошеного конкурентного діалогу
  ...      viewer
  ...      ${USERS.users['${viewer}'].broker}
  ...      minimal
...
Відображення закінчення періоду подання скарг на оголошений конкурентного діалогу
  [Tags]   ${USERS.users['${viewer}'].broker}: Відображення основних даних оголошеного конкурентного діалогу
  ...      viewer
  ...      ${USERS.users['${viewer}'].broker}
  ...      minimal
...
Подати цінову пропозицію першим учасником після оголошення конкурентного діалогу
...
Можливість редагувати однопредметний конкурентний діалог більше ніж за 7 днів до завершення періоду подання пропозицій
  [Tags]   ${USERS.users['${tender_owner}'].broker}: Можливість оголосити конкурентний діалог
  ...      tender_owner
  ...      ${USERS.users['${tender_owner}'].broker}
  [Teardown]  Оновити LAST_MODIFICATION_DATE
...
Неможливість редагувати однопредметний конкурентний діалог менше ніж за 7 днів до завершення періоду подання пропозицій
  [Tags]   ${USERS.users['${tender_owner}'].broker}: Можливість оголосити конкурентний діалог
  ...      tender_owner
  ...      ${USERS.users['${tender_owner}'].broker}
...
Продовжити період редагування подання пропозиції на 7 днів
  [Tags]   ${USERS.users['${tender_owner}'].broker}: Можливість продовжити період подання пропозицій конкурентного діалогу
  ...      tender_owner
  ...      ${USERS.users['${tender_owner}'].broker}
  [Teardown]  Оновити LAST_MODIFICATION_DATE
...
Можливість редагувати однопредметний конкурентний діалог після продовження періоду подання пропозицій
  [Tags]   ${USERS.users['${tender_owner}'].broker}: Можливість оголосити тендеконкурентний діалогр
  ...      tender_owner
  ...      ${USERS.users['${tender_owner}'].broker}
  [Teardown]  Оновити LAST_MODIFICATION_DATE
  Викликати для учасника   ${tender_owner}  Внести зміни в тендер  ${TENDER['TENDER_UAID']}   description     description
...
Відображення статусу першої пропозиції кваліфікації
  [Tags]   ${USERS.users['${tender_owner}'].broker}: Відображення основних даних оголошеного конкурентного діалогу
  ...      tender_owner
  ...      ${USERS.users['${tender_owner}'].broker}
