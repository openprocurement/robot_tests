...
Можливість додати предмет закупівлі
  ...
  ...         ELSE IF  '${mode}'=='with_lots'  Можливість додати предмет закупівлі в лот
...
Можливість видалити предмет закупівлі
  ...
  ...         ELSE IF  '${mode}'=='with_lots'  Можливість видалити предмет закупівлі з лоту
...
Відображення бюджету першого лоту
  ...
  ...      lots
  Відображення value.amount першого лоту для користувача ${viewer}
...
Можливість задати запитання на тендер
  ...
  [Teardown]  Оновити LAST_MODIFICATION_DATE
  Можливість задати запитання на тендер  ${provider}
...
Можливість задати запитання на предмет
  [Tags]   ${USERS.users['${provider}'].broker}: Можливість задати запитання
  ...      provider
  ...      ${USERS.users['${provider}'].broker}
  [Setup]  Дочекатись синхронізації з майданчиком  ${provider}
  [Teardown]  Оновити LAST_MODIFICATION_DATE
  Можливість задати запитання на предмет  ${provider}
...
Можливість задати запитання на лот
  ...
  [Setup]  Дочекатись синхронізації з майданчиком    ${provider}
  [Teardown]  Оновити LAST_MODIFICATION_DATE
  Можливість задати запитання на лот  ${provider}
...
Можливість подати пропозицію першим учасником
  ...
  ...         ELSE IF  '${mode}'=='with_lots'  Можливість подати цінову пропозицію на лоти  ${provider}
...
Можливість подати пропозицію другим учасником
  ...
  ...         ELSE IF  '${mode}'=='with_lots'  Можливість подати цінову пропозицію на лоти  ${provider1}
