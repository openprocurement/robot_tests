...
Відображення заголовку анонімного питання без відповіді
  ...
  Звірити поле тендера із значенням  ${viewer}  ${USERS.users['${provider}'].question_data.question.data.title}  title  ${USERS.users['${provider}'].question_data.question_id}
...
Відображення опису анонімного питання без відповіді
  ...
  ...      ${USERS.users['${viewer}'].broker}
  ...      critical level 2
  Звірити поле тендера із значенням  ${viewer}  ${USERS.users['${provider}'].question_data.question.data.description}  description  ${USERS.users['${provider}'].question_data.question_id}
...
Відображення дати анонімного питання без відповіді
  ...
  ...      viewer
  ...      ${USERS.users['${viewer}'].broker}
  Звірити дату тендера із значенням  ${viewer}  ${USERS.users['${provider}'].question_data.question.data.date}  date  ${USERS.users['${provider}'].question_data.question_id}
...
Можливість відповісти на запитання
  ...
  ${answer_resp}=  Викликати для учасника  ${tender_owner}  Відповісти на питання  ${TENDER['TENDER_UAID']}  ${USERS.users['${provider}']['question_data']['question_resp']}  ${answer}  ${USERS.users['${provider}'].question_data.question_id}
  ${now}=  Get Current TZdate
...
Відображення відповіді на запитання
  ...
  [Setup]  Дочекатись синхронізації з майданчиком  ${viewer}
  Викликати для учасника  ${viewer}  Оновити сторінку з тендером  ${TENDER['TENDER_UAID']}
  Звірити поле тендера із значенням  ${viewer}  ${USERS.users['${provider}']['answer_data']['answer'].data.answer}  answer  ${USERS.users['${provider}'].question_data.question_id}
