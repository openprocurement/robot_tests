...
Неможливість редагувати однопредметний тендер менше ніж за 7 днів до завершення періоду подання пропозицій
  ...
  ...      tender_owner
  ...      ${USERS.users['${tender_owner}'].broker}
  ${no_edit_time}=  get_future_date  ${USERS.users['${tender_owner}'].tender_data.data.tenderPeriod.startDate}  8
...
Продовжити період редагування подання пропозиції на 7 днів
  ...
  ...      tender_owner
  ...      ${USERS.users['${tender_owner}'].broker}
  ${endDate}=  get_future_date   ${USERS.users['${tender_owner}'].tender_data.data.tenderPeriod.endDate}  7
  Викликати для учасника   ${tender_owner}  Внести зміни в тендер  ${TENDER['TENDER_UAID']}   tenderPeriod.endDate     ${endDate}


Можливість подати скаргу на умови більше ніж за 4 днів до завершення періоду подання пропозицій
