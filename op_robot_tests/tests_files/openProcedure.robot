...
Відображення назви нас. пункту доставки активів лоту
...
Відображення locality адреси доставки активів лоту
  [Tags]   ${USERS.users['${viewer}'].broker}: Відображення номенклатури тендера
  ...      viewer
  ...      ${USERS.users['${viewer}'].broker}
  ...      tender_view
...
Перевірити тривалість періоду редагування лоту
...
Можливість змінити назву лоту українською мовою
  ...
  ${new_title}=  create_fake_title  UKR
...
Можливість змінити назву лоту російською мовою
  ...
  [Setup]  Дочекатись синхронізації з майданчиком  ${tender_owner}
  ${new_title}=  create_fake_title  RUS
  Set To Dictionary  ${USERS.users['${tender_owner}']}  new_title=${new_title}
...
Можливість змінити назву лоту англійською мовою
  ...
  ...      modify_auction_title  level3
  [Setup]  Дочекатись синхронізації з майданчиком  ${tender_owner}
  ${new_title}=  create_fake_title  ENG
  Set To Dictionary  ${USERS.users['${tender_owner}']}  new_title=${new_title}
...
Неможливість змінити номер лоту ФГВ після завершення періоду редагування лоту
  ...
  [Setup]  Дочекатись синхронізації з майданчиком  ${tender_owner}
  ${new_dgfID}=  create_fake_dgfID
  Перевірити неможливість зміни поля tenderAttempts тендера на значення ${new_dgfID} для користувача ${tender_owner}
