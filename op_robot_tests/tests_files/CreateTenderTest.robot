...
Можливість оголосити однопердметний тендер
  [Tags]   Owner_Test   ${USERS.users['${tender_owner}'].broker}: Пошук тендера по ідентифікатору
...
Відображення заголовоку оголошеного тендера
  [Tags]   Owner_Test     ${USERS.users['${tender_owner}'].broker}: Запис основних даних оголошеного тендера
...
Відображення опису позицій закупівлі однопредметного тендера
  [Tags]   Owner_Test   ${USERS.users['${tender_owner}'].broker}: Запис полів пердметів однопредметного тендера
