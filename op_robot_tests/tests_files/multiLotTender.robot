...
Неможливість змінити цінову пропозицію до 10 після закінчення прийому пропозицій
  [Documentation]
  ...    "shouldfail" argument as first switches the behaviour of keyword and "Викликати для учасника" to "fail if passed"
  [Tags]   ${USERS.users['${provider1}'].broker}: Неможливість змінити цінову пропозицію до 1 після закінчення прийому пропозицій
  ...      provider1
  ...      ${USERS.users['${provider1}'].broker}
  ${failfixbidto10resp}=  create_data_dict   data.lotValues[0].value.amount  10
  ${failfixbidto10resp}=  Викликати для учасника   ${provider1}   Змінити цінову пропозицію  shouldfail  ${TENDER['TENDER_UAID']}   ${failfixbidto10resp}
  Set To Dictionary  ${USERS.users['${provider1}'].bidresponses}   failfixbidto10resp   ${failfixbidto10resp}
  log  ${failfixbidto10resp}
