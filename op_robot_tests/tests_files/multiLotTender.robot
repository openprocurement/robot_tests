...
Resource        brokers/openprocurement_client.robot
...
Пошук мультилотового тендеру по ідентифікатору
...
Добавлення лоту
  ...
  Set Global Variable  ${lotresponses}
...
Добавити предмет закупівлі до лоту
  Log   ${tender_data.data['items']}
  ${items}=  Get Variable Value  ${tender_data.data['items']}
...
Можливість видалити позиції закупівлі тендера
  ...
  Викликати для учасника   ${tender_owner}  Внести зміни в тендер    ${TENDER['TENDER_UAID']}   items     ${items[:-1]}
  Log  ${tender_data.data}
...
Неможливість видалення лоту з прив’язаними документами
  ${lot}=  Get Variable Value  ${USERS.users['${tender_owner}'].lotresponses['resp']}
  Викликати для учасника   ${tender_owner}  Видалити лот  shouldfail   ${tender_data}  ${lot}
...
#  Викликати для учасника   ${provider}   Подати скаргу    ${TENDER['TENDER_UAID']}   ${COMPLAINTS[${complain_id}]}
#  ${LAST_MODIFICATION_DATE}=  Get Current Date
...
#  Викликати для учасника   ${provider}   Порівняти скаргу  ${TENDER['TENDER_UAID']}   ${COMPLAINTS[${complain_id}]}
#
#Можливість побачити скаргу анонімом
#  [Tags]   ${USERS.users['${viewer}'].broker}: Відображення основних даних оголошеного тендера
#  Викликати для учасника    ${viewer}  Порівняти скаргу  ${TENDER['TENDER_UAID']}   ${COMPLAINTS[${complain_id}]}
#
...
#  Викликати для учасника   ${tender_owner}   Обробити скаргу    ${TENDER['TENDER_UAID']}  0  ${COMPLAINTS[${complain_id}]}
#  log many   ${COMPLAINTS[${complain_id}]}
...
#  Викликати для учасника   ${tender_owner}   Обробити скаргу    ${TENDER['TENDER_UAID']}  1  ${COMPLAINTS[${complain_id}]}
#  log many   ${COMPLAINTS[${complain_id}]}
#  ${LAST_MODIFICATION_DATE}=  Get Current Date
#  Set Global Variable   ${LAST_MODIFICATION_DATE}
# Викликати для учасника   ${viewer}   Оновити сторінку з тендером   ${TENDER['TENDER_UAID']}
...
#  Викликати для учасника   ${tender_owner}   Обробити скаргу    ${TENDER['TENDER_UAID']}  2  ${COMPLAINTS[${complain_id}]}
#  log many   ${COMPLAINTS[${complain_id}]}
#  ${LAST_MODIFICATION_DATE}=  Get Current Date
#  Set Global Variable   ${LAST_MODIFICATION_DATE}
#
