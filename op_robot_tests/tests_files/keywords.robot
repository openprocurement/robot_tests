...
Звірити поле скарги про виправлення визначення переможця із значенням
  ...
  ${received_value}=  Отримати дані із скарги про виправлення визначення переможця  ${username}  ${award_index}  ${complaintID}  ${field_name}  ${given_value}
  Порівняти об'єкти  ${given_value}  ${received_value}


Отримати дані із скарги про виправлення визначення переможця
  [Arguments]  ${username}  ${award_index}  ${complaintID}  ${field_name}  ${given_value}
